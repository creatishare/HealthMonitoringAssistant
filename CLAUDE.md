...
- **2026-04-19**: UI响应式适配（手机+桌面双布局）、生产Docker部署配置（docker-compose + nginx + 部署脚本）、服务器运维文档

### 当前已知问题（下次开发优先处理）

#### 1. Dashboard 血压打卡卡片文字溢出（P0）
**现象**：今日打卡中血压数值（如 "120/70"）在小屏手机上超出卡片边界。
**已尝试**：将 `text-metric` (28px) 改为 `text-xl` (20px) + `whitespace-nowrap`，部署后仍溢出。
**待排查**：Tailwind 自定义配置优先级问题，或需进一步缩小字体/改布局。
**文件**：`src/frontend/src/pages/Dashboard.tsx`
**建议方案**：改用 `text-sm`（14px）、血压拆两行、或今日打卡改为 2 列布局。
...
**已知问题**：
- 后端服务当前未运行，修改未生效（Dashboard API 未返回新字段）
- 用户反馈"收起/展开"交互体验未达预期，需要重新设计
- **下一步**：重启后端验证数据流 → 重新设计指标展示交互（可考虑 Tab 分组、优先级折叠等方案）

### 开放待办 (按优先级)

| # | 优先级 | 任务 | 说明 | 状态 |
|---|--------|------|------|------|
| 1 | P0 | **血压打卡卡片文字溢出** | Dashboard 今日打卡血压值（"120/70"）在小屏手机超出卡片。已尝试改小字体仍溢出，需改布局或进一步缩小。 | ❌ 待修复 |
| 2 | P0 | **生产环境Redis** | 验证码仍存内存Map，重启丢失。需切换到Redis。 | ❌ 未开始 |
| 3 | P0 | **SMS 404修复** | 部署后发送验证码返回404，需检查后端路由。 | ❌ 待修复 |
| 4 | P1 | **Dashboard指标个性化** | 根据userType+primaryDisease动态展示推荐指标。代码已部分实现，需重新设计交互。 | 🚧 进行中 |
| 5 | P1 | **UI响应式适配** | ~~仅手机480px~~ ✅ 已完成（2026-04-19）：手机100%自适应+桌面左侧边栏 | ✅ 已完成 |
| 6 | P1 | **生产环境部署** | ~~待部署~~ ✅ 已完成（2026-04-19）：Docker Compose + nginx已部署到ECS | ✅ 已完成 |
| 7 | P2 | **健康洞察增强** | 接入每日打卡数据（血压、体重）到洞察引擎；图表联动 | ❌ 未开始 |
| 8 | P2 | **检查报告到期提醒** | 基于用户类型和上次检查日期，智能提醒复查时间 | ❌ 未开始 |
| 9 | P2 | **商业化付费功能** | 内测通过后实施。完整方案见 `docs/billing-plan.md`。 | ❌ 未开始 |
