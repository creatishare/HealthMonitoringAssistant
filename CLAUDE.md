...
| 3 | **Dashboard指标个性化** | 根据用户类型+原发疾病动态展示推荐指标。代码已部分实现，需重新设计交互后继续。 | 🚧 进行中 |
| 4 | **健康洞察增强** | 接入每日打卡数据（血压、体重）到洞察引擎；图表联动 | ❌ 未开始 |
| 5 | **检查报告到期提醒** | 基于用户类型和上次检查日期，智能提醒复查时间 | ❌ 未开始 |
| 6 | **商业化付费功能** | 内测通过后实施。完整方案见 `docs/billing-plan.md`，包含技术架构、支付集成、合规要求。 | ❌ 未开始 |
