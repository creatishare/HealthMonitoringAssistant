  ...
  if (!record) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-text-secondary">记录不存在</p>
        <button
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            aria-label="删除记录"
            className="flex h-11 w-11 items-center justify-center rounded-full text-danger transition-colors hover:bg-red-50/80 dark:hover:bg-red-950/20"
          >
            <Trash2 size={20} />
          </button>
        </div>
      ...
      <div className="card">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-small text-gray-text-secondary">记录日期</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {visibleMetrics.map((metric) => (
              <div key={metric.key} className="p-3 bg-gray-bg rounded-lg">
                <p className="text-small text-gray-text-secondary">{metric.label}</p>
                <div className="flex items-baseline gap-1">
                  <p className="text-metric text-gray-text-primary">
                    {metric.value}
                  </p>
                  <span className="text-small text-gray-text-secondary">{metric.unit}</span>
                </div>
      ...
      {cleanNotes && (
        <div className="card">
          ...
          <p className="text-body text-gray-text-secondary whitespace-pre-line">{cleanNotes}</p>
        </div>
      )}
